 ...
 *
 * ViewCache sınıfı - Görünüm önbellek sistemi
 */

require_once __DIR__ . '/CacheInterface.php';

class ViewCache implements CacheInterface
{
    /**
     * Önbellek dizini
     */
    ...
    public function __construct($cacheDir = null, $useEncryption = false, $cryptor = null)
    {
        ...
        if ($cacheDir === null) {
            $cacheDir = dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'views';
        }
    ...
    {
        return $this->cacheDir . md5($key) . '.view.cache';
    }
     ...
     */
    public function cacheView($viewFile, array $data = [], $ttl = 3600)
    {
        // Görünüm dosyasının hash'ini oluştur
        $key = md5($viewFile . serialize($data));
    ...
    /**
     * Görünüm dosyasının önbelleğini siler
     *
     * @param string $viewFile Görünüm dosyası
     * @param array $data Görünüm verileri
     * @return bool Başarılı mı?
     */
    public function clearView($viewFile, array $data = [])
    {
        $key = md5($viewFile . serialize($data));
